    ...
    module Services
      class VendorOne
        ...
        def retrieve_record_for_patient(vendor_id)
          ...
          Integrations::Models::VendorRecordResult.success VendorRecord.from_vendor_one_response(response.json)
        end
        ...
        def auth_token
          ...
          @auth_token = auth_token_response.success? ? auth_token_response.json[:token] : nil
        end

        # @return [Vandelay::Integrations::Util::RestResponse]
        def fetch_auth_token
          @rest_client.get('/auth/1')
        end
